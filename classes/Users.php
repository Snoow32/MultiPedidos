...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Detalhes do usuario salvo com sucesso.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
				...
				if(!empty($_FILES['img']['tmp_name'])){
					...
					if(!in_array($_FILES['img']['type'],$accept)){
						$err = "Tipo do arquivo de imagen é invalido.";
					}
			...
			if($qry){
				$this->settings->set_flashdata('success','Detalhes do usuario atualizado com sucesso.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
						if($this->settings->userdata('id') == $id)
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Usuario deletado com sucesso.');
	...
	function registration(){
		...
		if($save){
			...
			$resp['uid'] = $uid;
			if(!empty($id))
				$resp['msg'] = 'Detalhes do usuario foi atualizado com sucesso.';
			else
				$resp['msg'] = 'Sua conta foi criada com sucesso.';
			...
			if(!empty($_FILES['img']['tmp_name'])){
				...
				$accept = array('image/jpeg','image/png');
				if(!in_array($_FILES['img']['type'],$accept)){
					$resp['msg'] = "Tipo de imagem, invalida";
				}
				if($_FILES['img']['type'] == 'image/jpeg')
					$uploadfile = imagecreatefromjpeg($_FILES['img']['tmp_name']);
				elseif($_FILES['img']['type'] == 'image/png')
	...
	public function delete_customer(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Destalhes customizados foram deletados com sucesso.');
			$resp['status'] = 'success';
