...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    if($qry->num_rows > 0){
		...
		echo '<script>alert("ID da categoria não é valido."); location.replace("./?page=categoryes")</script>';
	}
}else{
	echo '<script>alert("Nescessario ID da categoria."); location.replace("./?page=categoryes")</script>';
}
?>
<style>
