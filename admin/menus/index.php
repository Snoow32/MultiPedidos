        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT m.*, c.name as `category` from `menu_list` m inner join subcategory_list c on m.category_id = c.id where m.delete_flag = 0 order by m.`code` asc, m.`name` asc ");
...
<script>
	$(document).ready(function(){
		...
		$('#create_new').click(function(){
			uni_modal("<i class='far fa-plus-square'></i> Adiciona um novo item ","menus/manage_menu.php")
		})
		$('.edit-data').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Adicionar novo item ","menus/manage_menu.php?id="+$(this).attr('data-id'))
		})
		$('.view-data').click(function(){
			uni_modal("<i class='fa fa-th-list'></i> Detalhes do item ","menus/view_menu.php?id="+$(this).attr('data-id'))
		})
		$('.table').dataTable({
			columnDefs: [
