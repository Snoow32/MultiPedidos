	...
	<form action="" id="menu-form">
		...
		<div class="form-group">
			...
			<select name="category_id" id="category_id" class="form-control form-control-sm rounded-0" required="required">
				...
				<?php 
				$categories = $conn->query("SELECT * FROM `subcategory_list` where delete_flag = 0 and `status` = 1 ");
			...
			$.ajax({
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						...
						uni_modal("<i class='fa fa-th-list'></i> Detalhes dos itens ","menus/view_menu.php?id="+resp.iid)
