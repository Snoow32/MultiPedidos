...
<div class="card card-outline rounded-0 card-warning">
	<div class="card-body">
		<div class="container-fluid">
			...
			<form action="" id="manage-user">	
				...
				<div class="form-group">
					<label for="password"><?= isset($meta['id']) ? "Nova" : "" ?> Senha</label>
					<input type="password" name="password" id="password" class="form-control" value="" autocomplete="off">
                    <?php if(isset($meta['id'])): ?>
					<small><i>Não deixe a senha vazia (por questões de segurança!)</i></small>
                    <?php endif; ?>
				</div>
                <div class="form-group">
                    ...
                    <option value="2" <?php echo isset($meta['type']) && $meta['type'] == 2 ? 'selected' : '' ?>>PDV(adicionar)</option>
                    <option value="3" <?php echo isset($meta['type']) && $meta['type'] == 3 ? 'selected' : '' ?>>Cozinha</option>
                    </select>
                </div>
				<div class="form-group">
	...
	<div class="card-footer">
			<div class="col-md-12">
				<div class="row">
					<button class="btn btn-sm btn-primary rounded-0 mr-3" form="manage-user">Salvar detalhes do usuario</button>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp ==1){
					...
					$('#msg').html('<div class="alert alert-danger">Esse usuario ja está em uso.</div>')
					end_loader()
				}
			}
		})
